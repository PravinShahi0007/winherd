 ...
 ******** NOTE - ANIMAL GRID HIGHLIGHT ********
 ...
 10/08/21 [V6.0 R1.8] /MK Additional Feature - Added BVD Result column - Alec (Flecthers) request.

 30/08/21 [V6.0 R2.1] /MK Additional Feature - Added Expected Calving Date and Expected Bull Code - Pat Ryan (Captal).
                                             - actRefreshAnimalsExecute - If Expected Calving Date in selected then show Due To Calve Report with message.

 06/09/21 [V6.0 R2.1] /MK Change - Changed Expected Calving and Bull Code to Projected Calving and Bull Code - GL request.

 07/09/21 [V6.0 R2.2] /MK Change - pmiAHDBFile - Changed caption to AHDB SCI/ACI Data as the AHDB import now includes the ACI data.

 09/09/21 [V6.0 R2.2] /MK Additional Feature - Added ACI column to grid that is vis
                          Change - ColumnHeaderClick - Added procedure to reset the font on the column like other previous columns.
                                 - Added ColumnHeaderClick as the OnHeaderClick for new columns that I add recently where this procedure was never added.
                          Bug Fix - If EBI column is hidden the user has no way of getting it back.
...
type
  ...
  TMenuForm = class(TForm)
   ...
   //----------------- New Main Screen Buttons/Actions ---------------------//
    ...
    cxAnimalGridViewBVDResult: TcxGridDBColumn;
    cxAnimalGridViewProjectedCalvingDate: TcxGridDBColumn;
    cxAnimalGridViewProjectedBullCode: TcxGridDBColumn;
    cxAnimalGridViewACI: TcxGridDBColumn;
   //--------------------------------------//
...
begin
   ...
   if UPPERCASE(Preferences.ValueAsString[cGSReportLayoutStyle]) = 'OLDLAYOUT' then
      ...
      begin
         ...
         //   30/08/21 [V6.0 R2.1] /MK Additional Feature - Refresh Animal Grid if Due To Calve Report was ran to show ExpectedCalvingDate/ExpectedBullCode values.
         if ( WinData.RefreshOverGainPerDay ) or ( WinData.DueCalvingUpdated ) then
            begin
               try
               ...
               WinData.DueCalvingUpdated := False;
            end;
      end;
...
begin
      ...
      begin
         ...
         if FSelectedHerdType = htDairy then
            begin
               ...
               if ( cxAnimalGridViewProjectedCalvingDate.Visible ) then
                  begin
                     cxAnimalGridViewProjectedCalvingDate.Visible := True;
                     cxAnimalGridViewProjectedCalvingDate.Hidden := False;
                  end
               else
                  begin
                     cxAnimalGridViewProjectedCalvingDate.Visible := False;
                     cxAnimalGridViewProjectedCalvingDate.Hidden := False;
                  end;

               if ( cxAnimalGridViewProjectedBullCode.Visible ) then
                  begin
                     cxAnimalGridViewProjectedBullCode.Visible := True;
                     cxAnimalGridViewProjectedBullCode.Hidden := False;
                  end
               else
                  begin
                     cxAnimalGridViewProjectedBullCode.Visible := False;
                     cxAnimalGridViewProjectedBullCode.Hidden := False;
                  end;

               if ( cxAnimalGridViewEBI.Visible ) and ( SystemRegCountry <> NIreland ) then
                  begin
                  ...
                  begin
                     cxAnimalGridViewEBI.Visible := False;
                     cxAnimalGridViewEBI.Hidden := ( SystemRegCountry = NIreland );
                  end;

               if ( cxAnimalGridViewACI.Visible ) and ( SystemRegCountry in [England, Scotland, Wales] ) then
                  begin
                     cxAnimalGridViewACI.Visible := True;
                     cxAnimalGridViewACI.Hidden := False;
                  end
               else
                  begin
                     cxAnimalGridViewACI.Visible := False;
                     cxAnimalGridViewACI.Hidden := ( not(SystemRegCountry in [England, Scotland, Wales]) );
                  end;
            ...
            begin
               ...
               if ( cxAnimalGridViewProjectedCalvingDate.Visible ) then
                  begin
                     cxAnimalGridViewProjectedCalvingDate.Visible := False;
                     cxAnimalGridViewProjectedCalvingDate.Hidden := True;
                  end
               else
                  begin
                     cxAnimalGridViewProjectedCalvingDate.Visible := False;
               ...
               if ( cxAnimalGridViewProjectedBullCode.Visible ) then
                  begin
                     cxAnimalGridViewProjectedBullCode.Visible := False;
                     cxAnimalGridViewProjectedBullCode.Hidden := True;
                  end
               else
                  begin
                     cxAnimalGridViewProjectedBullCode.Visible := False;
               ...
               if ( cxAnimalGridViewEBI.Visible ) then
                  begin
                     cxAnimalGridViewEBI.Visible := False;
                     cxAnimalGridViewEBI.Hidden := True;
                  end
               else
                  begin
                     cxAnimalGridViewEBI.Visible := False;
               ...
               if ( cxAnimalGridViewACI.Visible ) then
                  begin
                     cxAnimalGridViewACI.Visible := False;
                     cxAnimalGridViewACI.Hidden := True;
                  end
               else
                  begin
                     cxAnimalGridViewACI.Visible := False;
   ...
   if ( FSelectedHerdType = htBeef ) then
      ...
      begin
         SetLength(FSelectedCalcGridCols,1);
         if ( InArray(cxAnimalGridViewProjectedCalvingDate.Caption,cDefaultMainGridCalcColumns) ) then
            FSelectedCalcGridCols[0] := cxAnimalGridViewProjectedCalvingDate.Caption;
      end;
end;
...
begin
   //   28/10/21 [V6.0 R2.7] /MK Additional Feature - Pass in the ShowAllAnimals ActiveFilterType if Show All Animals is on to show AIM Details for selected animal - GL request.
   if ( WinData.ShowAllAnimals ) then
      WinData.GoToAimAnimalDetails(WinData.UserDefaultHerdID, afShowAllAnimals, WinData.AnimalFileByIDID.AsInteger)
   else
...
begin
   ...
   try
      ...
      //   30/08/21 [V6.0 R2.1] /MK Additional Feature - If Expected Calving Date in selected then show Due To Calve Report with message.
      if ( InArray(cxAnimalGridViewProjectedCalvingDate.Caption,FSelectedCalcGridCols)) then
         begin
            WinData.ShowMainGridGridDueToCalvCalcMsg := True;
            DueToCalve2Click(Sender);
            cxAnimalGridView.DataController.BeginFullUpdate;
            WinData.AnimalFileByID.DisableControls;
            try
               WinData.AnimalFileByID.Close;
...
procedure TMenuForm.ColumnHeaderClick(Sender: TObject);
begin
   if ( Sender is TcxGridDBColumn ) then
      ResetColumnHeader((Sender as TcxGridDBColumn).Index);
end;
