{
   ...
   18/01/22 [V6.0 R3.8] /MK Change - AddCalvingInformation - Older version of calving event saved the CalvingId1 and CalvingId2 fields as the same AnimalId
                                                             Check to see if the tag number of subsequent calves in this EventId have the same tag number
                                                             and don't add them to the calving node if this is the case.
}
         ...
         for i := 0 to Length(CalfArray)-1 do
            begin
               ...
               try
                  ...
                  //   18/01/22 [V6.0 R3.8] /MK Change - Older version of calving event saved the CalvingId1 and CalvingId2 fields as the same AnimalId
                  //                                     Check to see if the tag number of subsequent calves in this EventId have the same tag number
                  //                                     and don't add them to the calving node if this is the case.
                  if ( (Length(CalfTag) > 0) and (Length(qCalves.FieldByName('NatIdNum').AsString) > 0) ) then
                     if ( qCalves.FieldByName('NatIdNum').AsString = CalfTag ) then Continue;
                  FChildNode := FDocument.createElement('natIdNo');
                  FChildNode.Set_text(FieldByName('NatIdNum').AsString);
                  CalfXmlNode.appendChild(FChildNode);
                  CalfTag := FieldByName('NatIdNum').AsString;
